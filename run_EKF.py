...
def prediction_step(x_t_prev, u_t, sigma_x_t_prev):
    ...
    R = np.empty((2, 2)) # prof shia says to make this whatever- identity? also check dimenssions
...
def calc_kalman_gain(sigma_x_bar_t, H_t):
    ...
    K_t = sigma_x_bar_t @ H_t.transpose() @  np.linalg.inv((H_t @ sigma_x_bar_t @ H_t) + sigma_z_t)
    """STUDENT CODE END"""
